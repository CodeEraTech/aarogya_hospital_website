...
<section class="inner-section"><div class="wrap">
    ...
    <div class="table-card"><table class="schedule-table"><thead><tr><th>Doctor</th><th>Speciality</th><th>Days</th><th>Published hours</th><th></th></tr></thead><tbody>
    @forelse($schedules as $schedule)<tr data-schedule-row><td data-label="Doctor"><strong>{{ $schedule->doctor->name }}</strong></td><td data-label="Speciality">{{ $schedule->doctor->speciality?->name ?: '—' }}</td><td data-label="Days">{{ $schedule->days_label }}</td><td data-label="Hours">{{ CarbonCarbon::parse($schedule->start_time)->format('h:i A') }} – {{ CarbonCarbon::parse($schedule->end_time)->format('h:i A') }}</td><td><a href="{{ route('home', ['book' => 1]) }}">Book <span aria-hidden="true">→</span></a></td></tr>@empty<tr><td colspan="5" class="schedule-empty">No OPD schedules are currently available.</td></tr>@endforelse
