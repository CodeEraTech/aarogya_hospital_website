...
<section class="inner-section"><div class="wrap">
    <div class="schedule-toolbar"><div><label for="schedule-search">Find a doctor or service</label><input id="schedule-search" type="search" placeholder="Search OPD schedule..."></div><a class="btn btn-primary" href="{{ route('appointment.create') }}">Request Appointment</a></div>
    ...
    @forelse($schedules as $schedule)<tr data-schedule-row><td data-label="Doctor"><strong>{{ $schedule->doctor->name }}</strong></td><td data-label="Service">{{ $schedule->doctor->speciality ?: '—' }}</td><td data-label="Days">{{ $schedule->days_label }}</td><td data-label="Hours">{{ Carbon\Carbon::parse($schedule->start_time)->format('h:i A') }} – {{ Carbon\Carbon::parse($schedule->end_time)->format('h:i A') }}</td><td><a href="{{ route('appointment.create') }}">Book <span aria-hidden="true">→</span></a></td></tr>@empty<tr><td colspan="5" class="schedule-empty">No OPD schedules are currently available.</td></tr>@endforelse
